...
public class MayinTarlasi implements MouseListener {
	...
	MayinTarlasi() {
		...
		openMine();
		// printl();
		// System.out.println(board.length * board[0].length-10);
	...
	public void GenerateMine() {
		...
		while (i < 10) {
			...
			System.out.println(randRow + "," + randCol + " mayin var");
			i++;
		}
	}
	...
	public void countting(int row, int col) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getButton() == 1) {
			...
			if (b.isMine()) {
				...
				if (openButtom == (board.length * board[0].length - 10)) {
					JOptionPane.showMessageDialog(frame, "Tebrikler mayin tarlasi oynunu kazandiniz.");
			...
			if (!b.isFlag()) {
				b.setIcon(new ImageIcon("flag.jpg"));
				b.setFlag(true);
			} else {
				b.setIcon(null);
				b.setFlag(false);
